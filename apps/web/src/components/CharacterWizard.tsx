...
const normalizeAttr = (a?: string | null) => {
  ...
  return ["brawn", "nerves", "smooth", "focus", "crime"].includes(k)
    ? (k as any)
    : null;
};
...
function asName(x: any): string {
  ...
  if (typeof x === "object") {
    ...
    if ("value" in x && (x as any).value != null)
      return String((x as any).value);
  }
  return "";
}
...
type Props = {
  initial?: Partial<CharacterDTO>;
  onComplete?: (created: CharacterDTO) => void;
};
...
export default function CharacterWizard({ initial, onComplete }: Props) {
  ...
  const [tropeAttribute, setTropeAttribute] = useState<AttrKey | undefined>(
    undefined
  );

  // Role attribute (when role provides attribute_options)
  const [roleAttribute, setRoleAttribute] = useState<AttrKey | undefined>(
    undefined
  );
  ...
  const [age, setAge] = useState<"Young" | "Adult" | "Old">(
    (initial?.age as any) ?? "Adult"
  );

  // Portrait file + local preview (object URL) for character image
  const [portraitFile, setPortraitFile] = useState<File | null>(null);
  const [portraitDataUrl, setPortraitDataUrl] = useState<string | null>(null);

  // Local handler to track image file + preview (but don't embed base64 in DTO)
  function onPortraitChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Use an object URL for preview – this is never stored in the DB
  ...
  const roleGrants = useMemo(
    () => (role ? roleGearGrants(role) : []),
    [role]
  );
  const tropeGrants = useMemo(
    () => (trope ? tropeGearGrants(trope) : []),
    [trope]
  );
  const allGrants = useMemo(
    () => [...roleGrants, ...tropeGrants],
    [roleGrants, tropeGrants]
  );

  // Per-grant selections: array parallel to allGrants, each entry is a list of selected catalog IDs
  const [selectedByGrant, setSelectedByGrant] = useState<string[][]>(() =>
    allGrants.map(() => [])
  );
  ...
  const npcSpecial =
    specialRole && String(role).toLowerCase().includes("n.p.c");

  // Role attribute options (regular roles)
  const roleAttrOptions =
    (roleDef?.attribute_options as AttrKey[] | undefined) || [];

  // Trope attribute options (prefer options from JSON; disabled for Special Roles)
  const tropeAttrOptions =
    (tropeDef?.attribute_options as AttrKey[] | undefined) || [];
  ...
  }, [specialRole, npcSpecial, roleAttrOptions, tropeAttrOptions, tropeAttribute]);
  ...
  const roleGainedAttrs = useMemo((): string[] => {
    ...
    if (!specialRole && typeof roleDef.attribute === "string") {
      return [
        normalizeAttr(String(roleDef.attribute)) ?? String(roleDef.attribute),
      ];
    }
  ...
  const roleFeats = useMemo(
    () => Array.from(new Set(roleDef?.feats || [])),
    [roleDef]
  );
  ...
  async function save() {
    try {
      ...
      // If a portrait file was chosen, upload it and store the returned URL
      if (portraitFile) {
    ...
    <div className="max-w-3xl mx-auto p-6 space-y-4">
      ...
      {step === "roleTrope" && (
        <Card title="Role & Trope">
          <Select
            ...
            value={role}
            onChange={setRole}
            options={["", ...DATA.roles.map((r) => r.name)]}
          />
          <Select
            label="Trope *"
            value={trope}
            ...
            <p className="text-sm mt-1 italic text-muted-foreground">
              {"Trope: " +
                (tropeDef.description ||
                  "No description available for this trope.")}
            </p>
          )}
              ...
              {tropeNeedsAttr && (
                <>
                  <Select
                    ...
                    onChange={(v) =>
                      setTropeAttribute((v || undefined) as AttrKey)
                    }
                  ...
                  {tropeAttribute &&
                    ...
                    preTropeDTO &&
                    (() => {
                      const attr = normalizeAttr(
                        tropeAttribute
                      ) as AttrKey | null;
                      if (!attr) return null;

                      const beforeTrope =
                        (preTropeDTO.attributes?.[attr] as number) ?? 0; // value WITHOUT the Trope +1
                      const withTrope =
                        (preBumpDTO.attributes?.[attr] as number) ?? 0; // value WITH the Trope +1 (capped)

                      // If pre-Trope is already at cap, the +1 is necessarily lost
                      const CAP = 3;
                      const lost =
                        beforeTrope >= CAP || withTrope === beforeTrope;
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <Select
                      ...
                      options={[
                        ...
                        typeof x === "string" &&
                        ["brawn", "nerves", "smooth", "focus", "crime"].includes(
                          x
                        )
                          ? x
                          : x
                    ...
                    <Select
                      ...
                      onChange={(v) => {
                        ...
                        setSpecialAttrs((prev) => {
                          ...
                          // enforce uniqueness
                          return Array.from(new Set(next)).slice(
                            0,
                            2
                          ) as AttrKey[];
                        });
                ...
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="rounded-xl border p-3">
                    ...
                    {roleDef?.skills?.length ? (
                      <ul
                        className="list-disc ml-5 text-sm leading-6"
                        style={{ columns: 2 }}
                      >
                  ...
                  <div className="rounded-xl border p-3">
                    ...
                    {tropeDef?.skills?.length ? (
                      ...
                      <div className="text-sm text-gray-500">
                        No skills listed for this trope.
                      </div>
                    )}
                  </div>
                </div>
      ...
      {step === "age" && (
        <Card title="Age">
          ...
          <p className="text-xs text-muted-foreground mt-2">
            Young: 2 feats + “Too Young to Die”; 2 Adrenaline/Luck. Adult: 3
            feats. Old: 4 feats and start with 2 lethal bullets.
          </p>
        </Card>
      )}
      ...
      {step === "feats" && (
        <Card title="Feats">
          {specialRole ? (
            <p className="text-sm">
              Pick <b>3</b> feats from the lists below (Special Role).
            </p>
          ) : age === "Young" ? (
            <p className="text-sm">
              <b>Young:</b> <i>Too Young to Die</i> is added automatically.
              Choose <b>1 Role feat</b> and <b>1 Trope feat</b>.
            </p>
          ) : age === "Adult" ? (
            <p className="text-sm">
              Pick <b>2 Role feats</b> and <b>1 Trope feat</b>.
            </p>
          ) : (
            <p className="text-sm">
              <b>Old:</b> Choose <b>4</b> feats total (minimum <b>2 Role</b> and{" "}
              <b>1 Trope</b>; the extra can be from either list).
            </p>
          )}
          ...
          <div className="grid grid-cols-2 gap-4 mt-3">
            <div>
              <h4 className="font-semibold text-sm mb-1">
                Role Feats
                {!specialRole && age === "Adult"
                  ? ` (${featRule.roleMin} required)`
                  : ""}
              </h4>
      ...
      {step === "skillBumps" && (
        <Card title="Extra Skill Points">
                  ...
                  <div key={attr} className="rounded border p-2">
                    <div className="font-semibold text-sm capitalize mb-1">
                      ...
                      <span className="tabular-nums">
                        {preBumpDTO.attributes[attr] as any}
                      </span>
                    </div>
          ...
          <SkillPicker
            ...
            max={specialRole ? 6 : 2}
            baseLevels={
              (preBumpDTO?.skills as Record<SkillKey, number>) || {}
            }
          />
        </Card>
      )}
        ...
        <Card title="Background, Flaw & Catchphrase">
          {specialRole && (
            <p className="text-xs text-muted-foreground mb-2">
              Special Role: if left blank, Job / Background will default to{" "}
              <b>{role}</b>.
            </p>
          )}
          <Datalist
          ...
          {allGrants.length ? (
            <>
              <p className="text-sm mb-2">
                Pick your starting gear. Some are automatically granted, others
                let you choose. You can still add custom items below.
              </p>
              ...
              <div className="space-y-4">
                {allGrants.map((grant, idx) => {
                  ...
                  if (grant.mode === "give") {
                    const names = toNames(
                      optionsForGrant({
                        ...grant,
                        mode: "choose",
                        of: { type: "ids", ids: grant.items },
                        count: grant.items.length,
                      } as any)
                    );
                    return (
                    ...
                    const toggle = (id: string) => {
                      setSelectedByGrant((prev) => {
                        ...
                        if (current.has(id)) {
                          ...
                          if (total + (item.cost ?? 0) > grant.amount)
                            return prev; // keep budget
                          current.add(id);
                        }
                        copy[idx] = [...current];
                        return copy;
                  ...
                  const label =
                    ("label" in (grant.of as any) &&
                      (grant.of as any).label) ||
                    (grant.of.type === "ids"
                      ? "Choose"
                      : grant.of.type === "kind"
                      ? `Choose a ${grant.of.kind}`
                      : grant.of.type === "ride"
                      ? "Choose a ride"
                      : "Choose");
              ...
              <div className="mt-4">
                <AddLine
                  ...
                  onAdd={(txt) =>
                    txt && setGearCustom((prev) => [...prev, txt])
                  }
                />
                {!!(grantPickedNames().length + gearCustom.length) && (
                  <ul className="list-disc ml-6 mt-2 text-sm">
                    {[...grantPickedNames(), ...gearCustom].map((g, i) => (
                      <li key={i}>{g}</li>
                    ))}
                  </ul>
                )}
            ...
            <p className="text-sm text-muted-foreground">
              No structured gear found for this Role/Trope. Add custom gear
              below.
            </p>
          )}
        </Card>
      ...
      <div className="flex gap-2 pt-2">
        {step !== "identity" && (
          <button className="border rounded px-3 py-2" onClick={back}>
            Back
          </button>
        )}
        {step !== "review" && (
          <button
            className="rounded px-3 py-2 bg-white text-black disabled:opacity-40"
            onClick={next}
...
function Preview({ dto }: { dto: CharacterDTO }) {
  return (
    <div className="text-sm grid gap-2">
      <div>
        <b>{dto.name}</b> — {dto.role}
        {dto.trope ? ` / ${dto.trope}` : ""} — {dto.age}
      </div>
      ...
      <div>
        Adrenaline/Luck: {dto.adrenaline ?? dto.luck} | Spotlight:{" "}
        {dto.spotlight} | Cash: {dto.cash}
      </div>
      <div className="mt-6">
      ...
      {!!dto.storage?.gunsAndGear?.length && (
        <div>
          <b>Gear</b>:{" "}
          {dto.storage.gunsAndGear.map((g) => g.name).join(", ")}
        </div>
      )}
    </div>
  );
