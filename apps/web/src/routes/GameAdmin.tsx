...
export default function GameAdmin() {
  ...
  async function load() {
    setError(null);
    try {
      const data = await api(`/games/${gameId}/members`, { method: "GET" });
  ...
  async function setRole(userId: string, role: "director" | "hero") {
    ...
    setError(null);
    try {
      await api(`/games/${gameId}/members/${userId}/role`, {
        method: "POST",
        json: { role },
      });
      await load();
  ...
  return (
    <div className="max-w-3xl mx-auto p-4">
      <h1 className="text-2xl font-semibold mb-4">Director Admin</h1>
      ...
      <div className="rounded-lg border border-white/10 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {members.length === 0 && (
              <tr>
                <td className="px-3 py-6 text-center opacity-70" colSpan={4}>No members yet.</td>
              </tr>
            )}
          </tbody>
        </table>
