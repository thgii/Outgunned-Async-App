...
import { characterSchema, normalizeYouLook } from "@action-thread/types";
...
characters.post("/", async (c) => {
    ...
    await c.env.DB.prepare(`INSERT INTO characters
     ...
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(id, campaignId, ownerId, name, role, trope, age, job, catchphrase, flaw, tropeAttribute, feats, attributes, skills, resources, gear, conditions, notes, 1, createdAt).run();
    const row = await one(c.env.DB, "SELECT * FROM characters WHERE id = ?", [id]);
    for (const k of ["attributes", "skills", "resources", "feats", "gear", "conditions"]) {
        if (row[k])
            row[k] = JSON.parse(row[k]);
...
characters.patch("/:id", async (c) => {
    ...
    const conditions = JSON.stringify(dto.youLookSelected ?? []);
    const notes = dto.missionOrTreasure ?? null;
    await c.env.DB.prepare(`UPDATE characters SET
      name=?, role=?, trope=?, age=?, job=?, catchphrase=?, flaw=?, tropeAttribute=?,
      feats=?, attributes=?, skills=?, resources=?, gear=?, conditions=?, notes=?,
      revision=COALESCE(revision,0)+1
     WHERE id=?`).bind(name, role, trope, age, job, catchphrase, flaw, tropeAttribute, feats, attributes, skills, resources, gear, conditions, notes, id).run();
    const row = await one(c.env.DB, "SELECT * FROM characters WHERE id = ?", [id]);
    for (const k of ["attributes", "skills", "resources", "feats", "gear", "conditions"]) {
        if (row[k])
            row[k] = JSON.parse(row[k]);
